...
**套餐定位**:
- **Free**: 试用体验。手动浏览评论，基本 QR 分析。达到限额时提示升级。
- **Starter**: "我需要 AI 帮我"。AI 草稿 + 每日监控通知 + 情感分析 = 省时间的第一步。
- **Pro**: "我要全自动"。AI 自动回复 + 竞争对手分析 + 完整分析 = 核心目标层级。
- **Enterprise**: 销售驱动。无限门店 + API + SSO，面向连锁企业。

### 8.2 功能矩阵

| 功能 | Free | Starter | Pro | Enterprise |
|------|:----:|:-------:|:---:|:----------:|
| AI 评论生成（QR 流程） | Y (50/月) | Y (500/月) | Y (无限) | Y (无限) |
| 评论浏览与同步 | Y | Y | Y | Y |
| 基础 QR 扫描分析 | Y | Y | Y | Y |
| Dashboard 中控中心 | 基础 KPI | 完整（含洞察快照） | 完整（含洞察快照） | 完整（含洞察快照） |
| AI 回复草稿 | - | Y | Y | Y |
| 每日新评论监控通知 | - | Y | Y | Y |
| 情感分析 | - | Y | Y | Y |
| Review Insights（文字云 + AI 报告） | - | Y | Y | Y |
| 周报 | - | Y | Y | Y |
| 自动发布回复 | - | - | Y | Y |
| **自动回复模式** | - | - | Y | Y |
| **竞争对手分析** | - | - | Y | Y |
...
- **Free**: 获客。手动回覆 + 基本分析，让店家体验核心功能。达到限额时提示升级。
- **Starter**: "我需要 AI 帮我"。1 间店 + AI 草稿 + 每日监控 = 最低门槛的付费价值。
- **Pro**: "我要全自动"。3 间店 + 自动回复 + 竞争对手 + 完整分析 = **目标层级**。
- **Enterprise**: 销售驱动。无限门店 + API + SSO，面向连锁企业。定制定价。
...
**持续性价值（让店家离不开的功能）**:
1. **每日自动监控** — 新评论即时检测 + 通知。停用 = 错过负评。
2. **QR 好评引导** — 实体 QR 码放在店里，每天客人在扫。系统必须在线。
3. **负评即时警报** — 1 星评论 5 分钟内通知。不订阅就收不到。
4. **AI 自动回复** — 每天省 30+ 分钟。手动 = 痛苦。
5. **周报 + 洞察** — 持续性的运营情报。

**避免的模式**:
- 不以"回复数量限制"作为主要留存手段（会让用户反感）
- 一次性批量回复不构成持续价值
- 历史数据导入是一次性操作，不应作为付费卖点
...
| 2026-03-09 | 2.0 | 套餐重新定义：Free(1店/手动)→Starter(1店/AI草稿)→Pro(3店/自动回复+竞争对手)→Enterprise(无限/API/SSO)。Enterprise 暂不含白标。新增订阅留存策略。Dashboard 升级为中控中心（洞察快照+AI建议）。侧边栏精简 Analytics 9→3项。评论导入移除 90 天限制改为全量。Review Insights 新页面。 | 产品商业化准备 |
