export default async (request, context) => {
  try {
    ...
    const { OPENAI_API_KEY, SHEET_ID, SHEET_STORES = "stores", SHEET_HISTORY = "ReviewHistory", APPS_SCRIPT_APPEND_URL = "" } = process.env;

    const { storeid, selectedTags = [], lang = "zh-Hant", tone = "friendly", maxLen = 100 } = await request.json();
    if (!storeid) return new Response(JSON.stringify({ error: "storeid required" }), { status: 400 });
    if (!OPENAI_API_KEY) return new Response(JSON.stringify({ error: "Missing OPENAI_API_KEY" }), { status: 500 });
    if (!SHEET_ID) return new Response(JSON.stringify({ error: "Missing SHEET_ID env" }), { status: 500 });
    ...
    const row = stores.find(d => (d.StoreID || "").toLowerCase() === String(storeid).toLowerCase());
    if (!row) return new Response(JSON.stringify({ error: "store not found" }), { status: 404 });
    ...
    try {
      ...
      const recent = history.filter(h => (h.StoreID || "").toLowerCase() === String(storeid).toLowerCase()).slice(-20);
      lastSentences = recent.map(h => (h.GeneratedText || "").trim()).filter(Boolean);
    } catch (e) {}

    const system = "你是專業在地美食寫手。請以第一人稱，寫出真誠、口語、自然的短評，總字數控制在 50 到 100 字之間。禁止條列與數字列點，避免浮誇與不實承諾。盡量融合使用者選擇的關鍵字，同時與近期評論避免重複。";
    ...
    const user = [
      ...
      lastSentences.length ? `【記憶參考（請避免雷同）】：${lastSentences.join(" / ")}` : ""
    ].join("\n");
    ...
    if (process.env.APPS_SCRIPT_APPEND_URL) {
      try {
        await fetch(process.env.APPS_SCRIPT_APPEND_URL, {
          method: "POST",
